...
export const Sidebar: React.FC<SidebarProps> = ({ role }) => {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-slate-300 min-h-[calc(100vh-4rem)] border-r border-slate-800 transition-colors">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5">
          {navItems.map((item, idx) => {
            ...
            // Check if active (matches path exactly or via tab query parameter)
            const isActive = pathname + (window.location.search || "") === item.href || (pathname === item.href.split("?")[0] && !window.location.search && idx === 0);
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-slate-950 border-t border-gray-250 dark:border-slate-800 flex items-center justify-around h-16 px-2 shadow-lg transition-colors">
        {navItems.map((item) => {
          const Icon = item.icon;
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center flex-1 py-1 text-[10px] font-semibold transition-all
                ${isActive ? "text-brand-primary dark:text-brand-accent" : "text-gray-500 dark:text-gray-400"}`}
            >
              <Icon size={20} className={isActive ? "text-brand-primary dark:text-brand-accent scale-110" : "text-gray-400"} />
